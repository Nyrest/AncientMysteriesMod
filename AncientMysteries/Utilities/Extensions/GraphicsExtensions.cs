...
namespace AncientMysteries.Utilities
{
    public static class GraphicsExtensions
    {
        ...
        public static void DrawProgressBar(this Vec2 pos, Rectangle rect, float progress, Color back, Color fore, Color border, float borderWidth = 2f, Depth fuckingDepth = default)
        {
            Graphics.DrawRect(rect, border, fuckingDepth);
            var innerRect = new Rectangle(rect.x + borderWidth, rect.y + borderWidth, rect.width - borderWidth * 2, rect.height - borderWidth * 2);
            Graphics.DrawRect(innerRect, back, fuckingDepth + 1);
            innerRect.width *= progress;
            Graphics.DrawRect(innerRect, fore, fuckingDepth + 2);
        }
            ...
            float yOffset = 1f)
        {
            ...
            DrawProgressBar(new Vec2(thing.collisionCenter.x, thing.bottom), rect, progress, back, fore, border, borderWidth, thing.depth);
        }
